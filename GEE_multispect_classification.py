...
Original file is located at
    https://colab.research.google.com/drive/1AXBISoo8dKQHxjJ-b1MjebXI_AQXcfg6
...
ee.Initialize(project='helical-liberty-480011-i0')
...
"""__Note:__ "Minimum Distance" Classification algorithm is one of most simple and well-known classification algorithm. In this case study, this algorithm will first estimate centers in paddy and non-paddy classes in the feature space created for our example points. Then class of all other pixels in the image will be determined by minimum distance to centers of these 2 classes. As example, if a new pixel is more close to paddy class than non-paddy class, that new pixel will be assigned to paddy class, and vice-versa.
...
KrabiCol = ee.ImageCollection('COPERNICUS/S2_SR').filterDate('2020-01-01','2020-04-30').filterBounds(studyArea).select(['B4','B3','B2','B8'])
KrabiImg = KrabiCol.median().clip(studyArea)
...
"""__Exercise 1:__ Change *"nClusters=5"* parameter in "k means" clustering algorithm and compare the results.

__Exercise 2:__ Use "k means" clustering algorithm on *"KrabiImg"* (covering Krabi city) that we have used before, to classify landcover (Forest, Urban, Water and Agriculture) and calculate the total accuracy.

"""

studyArea = ee.Geometry.Rectangle(98.82, 7.98, 99.02, 8.18)

KrabiCol = ee.ImageCollection('COPERNICUS/S2_SR') \
...
Krabi_class5 = KrabiImg.cluster(my_clustering5)

Map = geemap.Map(center=[8.08,98.92], zoom=12)

Map.addLayer(Krabi_class5, {'min':0,'max':4}, "Clustered - 5 Classes")
...
my_clustering3 = ee.Clusterer.wekaKMeans(nClusters=3).train(Krabi_sample)

Krabi_class3 = KrabiImg.cluster(my_clustering3)

Map = geemap.Map(center=[8.08,98.92], zoom=12)

Map.addLayer(Krabi_class3.randomVisualizer(), {}, "Clustered - 3 Classes")

Map

""" "k means" clustering algorithm on Clusers K= 7"""

my_clustering7 = ee.Clusterer.wekaKMeans(nClusters=7).train(Krabi_sample)

Krabi_class7 = KrabiImg.cluster(my_clustering7)

Map = geemap.Map(center=[8.08,98.92], zoom=12)

Map.addLayer(Krabi_class7.randomVisualizer(), {}, "Clustered - 7 Classes")

Map

""""k means" clustering algorithm on Clusers K= 10"""

my_clustering10 = ee.Clusterer.wekaKMeans(nClusters=10).train(Krabi_sample)

Krabi_class10 = KrabiImg.cluster(my_clustering10)

Map = geemap.Map(center=[8.08,98.92], zoom=12)

Map.addLayer(Krabi_class10.randomVisualizer(), {}, "Clustered - 10 Classes")

Map

val_points = geemap.shp_to_ee('lc_gt_val.shp')
print(val_points.size().getInfo())

validation_cluster = Krabi_class5.sampleRegions(
    collection = val_points,
    properties = ['id'],
    scale = 10
)

val_accuracy = validation_cluster.errorMatrix('id','cluster')

print(val_accuracy.getInfo())

print("Overall Accuracy:", val_accuracy.accuracy().getInfo())

print("Kappa:", val_accuracy.kappa().getInfo())

"""Urban areas were found in **Clusters 0 and 1**, which shows that built-up areas have varied spectral characteristics. This is because urban surfaces often contain a mix of materials such as buildings, roads, and exposed soil.

Although the calculated overall accuracy is very low (1.7%), this mainly happens because the cluster numbers from K-means do not directly match the ground-truth class labels. K-means assigns cluster numbers randomly, so the confusion matrix counts a prediction as correct only when the cluster number happens to match the reference class ID. Because of this, the reported accuracy appears lower than the algorithm’s actual ability to separate different land cover types.
